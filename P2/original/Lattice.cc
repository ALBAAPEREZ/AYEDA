...
void Lattice::applyBorders(const BorderType& border, char* argv[]) {
  ...
  // Si el tipo de frontera es reflejante, ajusta los estados de las células en los bordes.
    ...
    cells_[rows_ - 1][0] = cells_[rows_ - 2][1];
    cells_[0][columns_ - 1] = cells_[1][columns_ - 2];
    cells_[rows_ - 1][columns_ - 1] = cells_[rows_ - 2][columns_ - 2];
    // Recorremos el retículo y ajustamos los estados de las células en los bordes.
    for (int i = 1; i < rows_ - 1; ++i) {
      cells_[i][0] = cells_[i][1];
      cells_[i][columns_ - 1] = cells_[i][columns_ - 2];
    }
    for (int i = 1; i < columns_ - 1; ++i) {
      cells_[0][i] = cells_[1][i];
      cells_[rows_ - 1][i] = cells_[rows_ - 2][i];
    }
 ...
 */
Lattice::Lattice(const BorderType& border, std::string& filename, char* argv[]) {
