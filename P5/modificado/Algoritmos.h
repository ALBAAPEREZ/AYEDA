...
void SelectionSortFunction(StaticSequence<Key>& seq, int size, bool trace) {
  int count = 0; // Variable para contar las comparaciones
  if (!trace) {
    std::cout << "Secuencia original: " << std::endl;
  ...
  std::cout << " >>>> Número de comparaciones: " << count << std::endl; // Imprimir el número de comparaciones
  if (trace) {
    std::cout << std::endl << "-------------------------------" << std::endl;
 ...
 */
template <class Key>
int partition(StaticSequence<Nif>& v, int low, int high, int& count) {
  Key pivot = v[high]; 
  int i = low - 1; 
  for (int j = low; j < high; j++) {
    count++; // Incrementar el contador de comparaciones
    if (v[j] <= pivot) {
      i++; 
      swap<Key>(v, i, j); 
    }
 ...
 */
template <class Key>
void quickSortAux(StaticSequence<Key>& v, int low, int high, bool traza, int& iteration, int& count) {
  if (low < high) {
    int pi = partition<Key>(v, low, high, count);
    quickSortAux<Key>(v, low, pi - 1, traza, iteration, count); 
    quickSortAux<Key>(v, pi + 1, high, traza, iteration, count); 
  }
  if (traza && low <= high) {
...
void quickSort(StaticSequence<Nif>& v, bool traza) {
  ...
  quickSortAux<Key>(v, low, high, traza, iteracion, count);
  std::cout << "Número de comparaciones: " << count << std::endl; // Imprimir el número de comparaciones
  if (traza) {
    std::cout << std::endl << "-------------------------------" << std::endl;
    std::cout << "Vector ordenado: ";
