...
class HashTable : public Sequence<Key> {
 public:
  ...
  // metodo que genera toda la tabla aleatoriamente
  void RandomTable(int num);
...
bool HashTable<Key, Container>::Insert(const Key& key) {
  ...
  if (table_[index].IsFull()) {
    ...
    while (full == true && i < tablesize_) {
      unsigned index = explorationfunction_->operator()(key, i);
      // si la secuencia no está llena, se busca en la secuencia correspondiente
      if (table_[index].IsFull() == false) {
        full = false;
      }
      i++;
    }
...
/**
 * @brief Método que genera un NIF aleatorio
 * Genera un NIF aleatorio con un número de identificación fiscal de 8 dígitos
 * @tparam Key clave que se va a dispersar
 * @return std::string retorna el NIF aleatorio
 */
template <class Key, class Container>
std::string HashTable<Key, Container>::generateRandomNif() {
  std::string nif;
  std::random_device rd;
  std::mt19937 gen(rd());
  std::uniform_int_distribution<> dis(0, 9);
  for (int i = 0; i < 8; i++) {
    nif += std::to_string(dis(gen));
  }
  return nif;
}

/**
 * @brief Método para generar una tabla aleatoria
 * Se encarga de generar una tabla aleatoia con un número de claves que se le pasa por parámetro
 * @tparam Key clave que se va a dispersar
 * @tparam Container clase que se va a usar para la secuencia
 * @param numberOfKeys número de claves que se van a generar
 */
template <class Key, class Container>
void HashTable<Key, Container>::RandomTable(int numberOfKeys) {
  for (int i = 0; i < numberOfKeys; ++i) {
    Key key = Nif();
    unsigned hash = this->dispersionfunction_->operator()(key);
    unsigned position = this->explorationfunction_->operator()(key, i);  // Asume que operator() acepta dos argumentos
    this->table_[position].Insert(key);
  }
}
...
void HashTable<Key, Container>::menu() {
  ...
  do {
    ...
    switch (opcion) {
      ...
      case 4: 
        RandomTable(500);
        std::cout << "Tabla generada aleatoriamente" << std::endl;
        break;
      case 5:
        std::cout << std::endl;
